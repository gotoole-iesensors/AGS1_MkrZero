...
#define Continous_Output_Period   1                   //Continuous output interval of 1s

#define AGS_Vdd                   3.3                 //(Dependent on voltage supply to AGS1)
#define ADC_Vref                  3.3                 //(Dependent on Arduino board)
#define ADC_Range                 1024                //(Dependent on Arduino board)
#define ADC_Range_Multiplier      64                  //(ADC_Range * ADC_Range_Multiplier = 65536)
...
#define ADC0_Zero                 31022               //Adjust this manually after reading EEPROM
#define ADC0_OC                   30987               //Adjust this manually after reading EEPROM
#define ADC1_Zero                 30374               //Adjust this manually after reading EEPROM
...
{  
  ...
  float PPB;
  float Temperature;
  float Temperature_Zero = ((float)87 / (float)AGS_Vdd * ((float)ADC1Result_Zero / ((float)ADC_Range * ADC_Range_Multiplier) * (float)ADC_Vref)) - (float)18;
  ...
  while(1)
  {
    if((millis() - LastMillis) > 1000)
    {
      ...
      ADC1Result = ADC1Result >> 0;

      Temperature = ((float)87 / (float)AGS_Vdd * ((float)ADC1Result / ((float)ADC_Range * ADC_Range_Multiplier) * (float)ADC_Vref)) - (float)18;
      
      nA = ((float)ADC0Result - (float)ADC0Result_OC) / ((float)ADC_Range * ADC_Range_Multiplier) * (float)ADC_Vref / (float)Rgain * 1E9;
      Zcf = ((float)ADC0Result_Zero - (float)ADC0Result_OC) / ((float)ADC_Range * ADC_Range_Multiplier) * (float)ADC_Vref / (float)Rgain * 1E9;    
      Zcf = Zcf * exp((Temperature - Temperature_Zero) / n_factor);  
      nA = nA - Zcf;  
      PPB = nA / nA_per_PPM * 1E3;
      
//      PPB = ((float)ADC0Result - (float)ADC0Result_Zero) / ((float)ADC_Range * ADC_Range_Multiplier) * (float)ADC_Vref / (float)Rgain * (float)1E9 / (float)nA_per_PPM * (float)1E3;
    ...
    if((millis() > (Timed_Initial_Zero_Period * 1000)) && (!Zero_Timer_Triggered)) 
    {
      ...
      Temperature_Zero = ((float)87 / (float)AGS_Vdd * ((float)ADC1Result_Zero / ((float)ADC_Range * ADC_Range_Multiplier) * (float)ADC_Vref)) - (float)18;
      Serial.println("************************************************************");
